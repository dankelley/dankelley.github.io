...
The code chunk given below is such a test, with the build-in dataset named ``co2``, which is a time-series starting in 1959.  Since any function in a package is necessarily documented, I will not bother with explaining things here.

Note that the code has a few tests on arguments provided to it.  Things are only known to work in the circumstance illustrated here, with ``x`` and ``t`` being provided.
